...
export default function ProjectPage() {
    ...
    return (
        <main className="p-8 lg:p-12 max-w-5xl mx-auto min-h-full">
            <motion.button
                initial={{ opacity: 0, x: -10 }}
                animate={{ opacity: 1, x: 0 }}
                onClick={() => router.push("/dashboard")}
                className="mb-8 flex items-center gap-2 text-gray-400 font-bold text-sm uppercase tracking-widest hover:text-blue-600 transition-colors group"
            >
                <span className="group-hover:-translate-x-1 transition-transform">←</span> Back to Dashboard
            </motion.button>

            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 px-2">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
                            <span className="text-sm font-bold">P</span>
                        </div>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Workspace Project</p>
                    </div>
                ...
                <button
                    ...
                    className="group bg-gray-900 text-white px-6 py-3 rounded-xl font-bold shadow-xl shadow-gray-900/10 hover:bg-gray-800 hover:scale-[1.02] transition-all active:scale-95 flex items-center gap-2 text-sm"
                >
                    <span className="text-xl font-light relative -top-0.5">+</span> 
                    <span>New Task</span>
                </button>
            </header>
            ...
            <AnimatePresence>
                {isCreatingTask && (
                    <motion.div
                        ...
                        className="mb-12 p-8 bg-white/50 backdrop-blur-xl rounded-[2rem] border border-blue-100 shadow-[0_20px_50px_rgba(0,0,0,0.03)]"
                    >
                        <form onSubmit={(e) => { e.preventDefault(); createTaskMutation.mutate(newTaskTitle); }} className="flex flex-col gap-6">
                            <div className="flex gap-4">
                                <input
                                    placeholder="Brief task description..."
                                    value={newTaskTitle}
                                    onChange={(e) => setNewTaskTitle(e.target.value)}
                                    className="flex-1 px-6 py-4 rounded-xl bg-white border border-gray-100 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 outline-none transition-all font-semibold text-gray-900"
                                    autoFocus
                                />
                                <button
                                    type="submit"
                                    disabled={createTaskMutation.isPending || !newTaskTitle.trim()}
                                    className="px-8 py-4 bg-blue-600 text-white rounded-xl font-bold shadow-xl shadow-blue-500/20 hover:bg-blue-700 disabled:opacity-50 transition-all active:scale-95"
                                >
                                    {createTaskMutation.isPending ? "Adding..." : "Create"}
                                </button>
                            </div>
                            <div className="flex justify-end px-4">
                                <button type="button" onClick={() => setIsCreatingTask(false)} className="text-[11px] font-bold text-gray-400 uppercase tracking-widest hover:text-red-500 transition-colors">
                                    Cancel
                                </button>
                            </div>
                        </form>
            ...
            <div className="flex flex-col gap-8">
                <div className="space-y-6">
                    <div className="flex items-center justify-between px-2 mb-2">
                        <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Workflow Queue</h2>
                        <span className="text-[11px] font-bold text-gray-400 bg-gray-100 px-2 py-1 rounded-md">{filteredTasks?.length || 0} tasks</span>
                    </div>
                    ...
                    {loadingTasks ? (
                        ...
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="text-center py-24 px-10 bg-gray-50/50 rounded-[3rem] border-2 border-dashed border-gray-200/50 flex flex-col items-center"
                        >
                            <div className="text-6xl mb-6 opacity-80">✨</div>
                            <h3 className="text-xl font-bold text-gray-900 mb-2">No obstacles found</h3>
                            <button
                                onClick={() => setIsCreatingTask(true)}
